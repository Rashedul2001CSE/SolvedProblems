...
/*--------------------------------------------------------------------------------------------------------------------------*/
ll dfs(int i, int j, int k, const vector<string> &mat, vector<vector<bool>> &vis, const int &n, const int &m)
{
    if (i < 0 or j < 0 or i == n or j == m or mat[i][j] == '#' or vis[i][j])
        return 0;
    vis[i][j] = true;
    if (k == 0)
    {
    ...
    ll ans = dfs(i + 1, j, k - 1, mat, vis, n, m)+ dfs(i - 1, j, k - 1, mat, vis, n, m) + dfs(i, j + 1, k - 1, mat, vis, n, m) + dfs(i, j - 1, k - 1, mat, vis, n, m);
    vis[i][j] = false;
    return ans;
}
