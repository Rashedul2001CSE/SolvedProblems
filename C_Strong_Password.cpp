...
void solve(int testCase)
{
    string str;
    cin >>str;
    vector<vector<int>> memo(10, vector<int>());
    rep2(i, str.size())
    {
        memo[str[i] - '0'].push_back(i);
    }
    int l;
    string low, high;
    cin >> l >> low >> high;
    int pre = -1;
    if (testCase == 16){
        cout << str << " " << l << " " << low << " " << high << " " << endl;
    }
    rep2(i, l)
    {
        for (int j = low[i] - '0'; j <= high[i] - '0'; j++)
        {
            auto it = lower_bound(memo[j].begin(), memo[j].end(), pre);
            if (it == memo[j].end())
            {
                cout << "YES\n";
                return;
