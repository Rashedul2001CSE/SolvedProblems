...
bool multiTest = true;
bool couldEaten(int n, int k)
{
    if (memo.find({n,k}) != memo.end()) return memo[{n,k}];
    if (n == 1) return (k == 10 or k ==11 or k == 12);
    else
        memo[{n,k}] = couldEaten(n - 1, k - 10) or couldEaten(n - 1, k - 11) or couldEaten(n - 1, k - 12);
        return memo[{n,k}];
}
