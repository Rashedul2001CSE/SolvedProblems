...
void solve(int testCase)
{
    ll n, m, l;
    cin >> n >> m >> l;
    vector<pair<ll, ll>> hurdles, powerUps;
    rep1(n)
    {
        ll x, y;
        cin >> x >> y;
    ...
    rep(i, hurdles)
    {
        if (i.ss + 2 > curPower)
        {
            while (powPos < m and powerUps[powPos].ff < i.ff)
            {
                pq.push(powerUps[powPos].ss);
                powPos++;
            }
            while (!pq.empty() and curPower < 2 + i.ss)
            {
                curPower += pq.top();
                pq.pop();
                ans++;
