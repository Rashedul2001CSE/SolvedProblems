...
ll mod_add(ll a, ll b, ll m) {a = a % m; b = b % m; return (((a + b) % m) + m) % m;} ll mod_mul(ll a, ll b, ll m) {a = a % m; b = b % m; return (((a * b) % m) + m) % m;} ll mod_sub(ll a, ll b, ll m) {a = a % m; b = b % m; return (((a - b) % m) + m) % m;}
...
void solve(int testCase)
{
    ...
    rep3(i,1,vec.size()){
        rep2(j,vec[i].size()){
        cout << vec[0][0] << " " << vec[i][j] << nline;
        }
    }
    rep3(i,1,vec[0].size()){
        cout << vec[0][i] << " " << vec[1][0] << nline;
    }
