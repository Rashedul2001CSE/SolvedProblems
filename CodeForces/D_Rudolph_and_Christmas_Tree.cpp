...
void solve(int testCase)
{
    ll n, d, h;
    cin >> n >> d >> h;
    lld angle = atan(h * 2.0L / d * 1.0L);
    lld totalArea = (n * d * h) / 2.0L;
    lld wasteArea = 0.0L;
    ll pre = 0;
    while (n--)
    {
        ll y;
        cin >> y;
        if (pre + h > y and pre != 0)
        {
            ll height = pre + h - y;
            lld base = height * 2.0L / tan(angle);
            wasteArea += base * height / 2.0L;
        }
        pre = y;
    }
    cout << prec(7) << totalArea - wasteArea << nline;
}
int main()
{
#ifdef RASHEDUL
