...
void solve(int testCase)
{
    ...
    ll maxGap = 0;
    for (int i = 1; i < n; i++) {
        maxGap = max(maxGap, lanterns[i] - lanterns[i - 1]);
    }
    ll maxDistance = max(lanterns[0], l - lanterns[n - 1]);
    maxGap = max(maxGap, 2 * maxDistance);
    double result = static_cast<double>(maxGap) / 2.0;
    cout<< prec(10) << result << nline;
