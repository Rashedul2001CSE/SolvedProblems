...
void solve(int testCase)
{
    int n;
    cin >> n;
    string baseStr, targetStr;
    cin >> baseStr >> targetStr;
    ...
    for (int i = 0; i < n; i++)
    {
        zero += (baseStr[i] == '0');
        one += (baseStr[i] == '1');
        preCount.push_back(make_pair(zero, one));
    }
    // _print(preCount);
    int operation = 0;
    for (int i = n - 1; i >= 0; i--){
        if ((baseStr[i] == targetStr[i] and operation % 2 == 0 )or (baseStr[i] != targetStr[i] and operation % 2 )){
            continue;
        }
        else{
            if (preCount[i].first != preCount[i].second)
            {
                cout << "NO\n";
                return ;
            }
