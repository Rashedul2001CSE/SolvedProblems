...
bool multiTest = true;
ll a,b,c,d,e,f,n,tempMod = 10000007;
map<ll,ll> memo;
ll fn(ll n) {
    ...
    if (n == 4) return e;
    if (n == 5) return f;
    memo[n] =  (fn(n-1)%tempMod + fn(n-2)%tempMod + fn(n-3)%tempMod + fn(n-4)%tempMod + fn(n-5)%tempMod + fn(n-6)%tempMod)%tempMod;
    return memo[n];
}
